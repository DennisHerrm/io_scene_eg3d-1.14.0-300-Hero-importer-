...
# Value type code from the channel descriptor, field [6][1].
# Measured over 136395 channels in 51 files: type 0 is float32 (position,
# rotation, scale, material colours) and type 6 is uint8 (the boolean tracks
# 'active', 'enable' and 'rendererenable'). Times are always type 5, uint16
# deltas in milliseconds.
CHANNEL_VALUE_TYPES = {0: ("<f4", 4), 6: ("<u1", 1)}

# Channel kinds that switch a node on or off rather than transforming it.
BOOLEAN_CHANNELS = ("active", "enable", "rendererenable")
...
class Channel(object):
    def __init__(self):
        ...
        self.values = None       # numpy (key_count, components)
        self.value_type = 0      # 0 = float32, 6 = uint8 (boolean tracks)
...
def read_container(path, log=None):
    ...
    if len(data) < 16:
        # A truncated file is a problem with the extraction, not with the
        # importer -- 020_skin3.model in one archive was a single byte, the
        # ASCII character '1', while its 1 MB texture had extracted fine.
        raise ParseError(
            "%s is only %d byte(s) long and cannot be a model. The file "
            "itself is truncated or empty -- extract it from the game again. "
            "(A valid .model starts with 'EG3D' and is at least a few kB.)"
            % (os.path.basename(path), len(data)))
    magic = data[0:4]
    if magic != b"EG3D":
        if data[:5] == b"JUMPX":
            raise ParseError(
                "%s is a JUMPX (.x) file that was named .model. The importer "
                "detects the format by signature, so just import it directly."
                % os.path.basename(path))
        raise ParseError(
            "%s does not start with 'EG3D' but with %r. Either it is not a "
            "300 Heroes model or the extraction produced garbage."
            % (os.path.basename(path), bytes(magic)))
...
def parse_clips(model, data, log=None):
    ...
    for ci, cd in enumerate(meta7):
        ...
        for chd in cd[1]:
            ...
            # chd[6][1] is the VALUE TYPE. Reading everything as float32 turns
            # the boolean tracks into garbage: an 'active' channel holding the
            # bytes 01 00 came out as 9.18e-41 instead of [1, 0].
            ch.value_type = chd[6][1] if len(chd[6]) > 1 else 0
            dtype, isize = CHANNEL_VALUE_TYPES.get(ch.value_type, ("<f4", 4))
            need = ch.key_count * ch.components * isize
            if v_off + need > len(data):
                if log:
            ...
            ch.values = np.frombuffer(
                data, dtype=dtype, count=ch.key_count * ch.components,

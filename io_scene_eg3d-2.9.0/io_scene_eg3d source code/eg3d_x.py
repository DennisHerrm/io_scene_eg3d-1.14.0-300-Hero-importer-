...
def parse_jumpx(path, log=None):
    ...
    # The animation address does NOT always sit at +144. Files from the
    # monster set put it at +148 -- the whole track table is shifted by four
    # bytes there. Find the offset that yields a valid block address for EVERY
    # bone instead of hard coding one.
    addr_off = None
    for cand in range(120, BONE_STRIDE - 4):
        good = True
        for k in range(nbon):
            q = abon + k * BONE_STRIDE + cand
            if q + 4 > len(B1):
                good = False
                break
            a = struct.unpack_from("<I", B1, q)[0] - ADDR_BASE
            if not (0 <= a < len(B2)):
                good = False
                break
        if good:
            ...
            break
    if addr_off is None:
        raise XParseError("no bone track address column found in the bone "
                          "descriptors (%d bones, stride %d)" % (nbon, BONE_STRIDE))
    if log and addr_off != 144:
        log.info("bone track addresses sit at +%d (not the usual +144)", addr_off)
    addrs, addrs2 = [], []
    for k in range(nbon):
        b0 = abon + k * BONE_STRIDE + addr_off
        addrs.append(struct.unpack_from("<I", B1, b0)[0] - ADDR_BASE)
        a2 = struct.unpack_from("<I", B1, b0 + 12)[0] - ADDR_BASE
        addrs2.append(a2 if 0 <= a2 < len(B2) else None)
    ...
    # There are TWO track addresses per bone: position first, rotation 12 bytes
    # later in the descriptor. Their DISTANCE is the size of the position
    # track, which gives the frame count directly:
    #     A2 - A1 = frames * 12   -> float32 positions
    #     A2 - A1 = frames *  4   -> positions packed 10:10:10 into a box
    # The monster set uses the packed form. This replaces guessing the frame
    # count from block sizes, which could not work there at all because the
    # blocks are not contiguous.
    deltas = {a2 - a1 for a1, a2 in zip(addrs, addrs2) if a2 is not None}
    model.frames = 0
    packed_pos = False
    unknown_rotation = False
    packed_scale = False
    if len(deltas) == 1:
        delta = next(iter(deltas))
        for per, packed in ((12, False), (4, True)):
            if delta % per:
                continue
            f = delta // per
            if f <= 0:
                continue
            share, dev = _unit_quaternion_score(addrs2, B2, f, rot_only=True)
            if dev < 0.005:
                model.frames, packed_pos = f, packed
                if log:
                    log.info("timeline: %d frames from the track spacing "
                             "(%d bytes per position key%s)", f, per,
                             ", 10:10:10 packed" if packed else "")
                break
    if not model.frames and len(deltas) == 1:
        # Compressed variant: positions 10:10:10 (4 bytes per key) and
        # rotations packed as axis-angle (8 bytes per key). A float32x3 scale
        # track may follow, which makes 24 bytes per frame in total --
        # tx_030_shenmujiaren_01.x is built that way.
        delta = next(iter(deltas))
        rot_bytes = min(sizes) - delta
        f = delta // 4 if delta % 4 == 0 else 0
        if f and rot_bytes in (f * 8, f * 20):
            model.frames = f
            packed_pos = True
            unknown_rotation = True
            packed_scale = (rot_bytes == f * 20)
            if log:
                log.info("compressed animation variant: positions 10:10:10, "
                         "rotations axis-angle (8 bytes per key)%s",
                         ", plus a float32 scale track" if packed_scale else "")
    ...
    if packed_pos:
        if log:
            log.info("bone positions are packed 10:10:10 per bone")
    ...
    for k in range(nbon):
        ...
        o = addrs[k]
        if packed_pos:
            base_k = abon + k * BONE_STRIDE + addr_off
            # The box floats sit 60 and 48 bytes ahead of the address column
            # (i.e. at +88 and +100 with the usual addr_off of 148), NOT 24/12.
            hi = np.asarray(struct.unpack_from("<3f", B1, base_k - 60),
                            dtype=np.float64)
            lo = np.asarray(struct.unpack_from("<3f", B1, base_k - 48),
                            dtype=np.float64)
            packed = np.frombuffer(B2, dtype="<u4", count=frames, offset=o)
            q = np.stack([((packed >> sh) & 1023).astype(np.float64) / 1023.0
                          for sh in (0, 10, 20)], axis=1)
            b.pos = (lo + q * (hi - lo)).astype(np.float32)
        else:
            b.pos = np.frombuffer(B2, dtype="<f4", count=frames * 3,
                                  offset=o).reshape(frames, 3)
        if unknown_rotation:
            ro = addrs2[k] if addrs2[k] is not None else o + frames * 8
            words = np.frombuffer(B2, dtype="<u8", count=frames, offset=ro)
            b.quat = _decode_packed_rotation(words)
            if packed_scale:
                b.scale = np.frombuffer(B2, dtype="<f4", count=frames * 3,
                                        offset=ro + frames * 8).reshape(frames, 3)
        else:
            ro = addrs2[k] if addrs2[k] is not None else o + frames * 12
            b.quat = np.frombuffer(B2, dtype="<f4", count=frames * 4,
                                   offset=ro).reshape(frames, 4)
        bpf = 0 if packed_pos else sizes[k] // frames
    ...
    for g in range(ngeo):
        ...
        # OLDER VARIANT (23 TOC entries instead of 25, e.g. 083_b.x):
        # positions are quantised to ONE uint32 per vertex as 10:10:10 and
        # dequantised against a bounding box stored in the descriptor, and
        # normals are int8 x3 packed into 3 bytes instead of 4. Detected by
        # the absence of a vc*12 position buffer at +36, with an address at
        # +40 instead. The 1023 mask sits at +120.
        quantised = a_pos is None and addr(40) is not None
        a_qpos = a_nrm3 = None
        qbox = None
        if quantised:
            a_qpos, a_nrm3 = addr(40), addr(48)
            hi = struct.unpack_from("<3f", B1, base + 96)
            lo = struct.unpack_from("<3f", B1, base + 108)
            if all(np.isfinite(hi + lo)) and any(h > l for h, l in zip(hi, lo)):
                qbox = (np.asarray(lo, dtype=np.float64),
                        np.asarray(hi, dtype=np.float64))
            else:
                quantised = False
            if log and quantised:
                log.info("mesh %d uses the older packed layout: 10:10:10 "
                         "positions in a box %s..%s, int8x3 normals", g,
                         np.round(lo, 2).tolist(), np.round(hi, 2).tolist())
        if quantised and a_qpos is not None and qbox is not None:
            packed = np.frombuffer(B2, dtype="<u4", count=vc, offset=a_qpos)
            lo, hi = qbox
            q = np.stack([((packed >> sh) & 1023).astype(np.float64) / 1023.0
                          for sh in (0, 10, 20)], axis=1)
            m = XMesh()
            ...
            m.material = matidx
            m.verts = (lo + q * (hi - lo)).astype(np.float32)
            if a_nrm3 is not None:
                n3 = np.frombuffer(B2, dtype="<i1", count=vc * 3,
                                   offset=a_nrm3).reshape(vc, 3).astype(np.float32)
                n3 = n3 / 127.0
                ln = np.linalg.norm(n3, axis=1, keepdims=True)
                m.normals = (n3 / np.maximum(ln, 1e-9)).astype(np.float32)
            else:
                m.normals = np.zeros((vc, 3), dtype=np.float32)
            a_pos = -1  # mark as handled
        else:
            m = None
    ...
    # Some files declare clips beyond their own timeline --
    # tx_030_shenmujiaren_01.x has 41 frames (its blocks are 41 * 24 bytes)
    # but an 'attack1' clip ending at frame 45. That is an authoring error in
    # the file, not a decoding problem, so clamp instead of refusing.
    clamped = 0
    for c in model.clips:
        if c.end >= model.frames:
            c.end = model.frames - 1
            clamped += 1
        if c.start > c.end:
            c.start = c.end
    for c in model.clips:
    ...
    if log and model.clips:
        log.info("clips: %d, highest frame %d of %d", len(model.clips),
                 max(c.end for c in model.clips), model.frames - 1)
        if clamped:
            log.warn("%d clip(s) declared frames past the end of the %d-frame "
                     "timeline and were clamped (error in the source file)",
                     clamped, model.frames)
    return model


def _bind_from_descriptor(B1, off):
    """World bind matrix from the inverse-bind stored in the bone descriptor."""
    if off < 0 or off + 64 > len(B1):
        return None
    M = np.asarray(struct.unpack_from("<16f", B1, off),
                   dtype=np.float64).reshape(4, 4).T
    if not np.isfinite(M).all() or abs(np.linalg.det(M)) < 1e-9:
        return None
    R = M[:3, :3]
...
def _matrix_to_quat(R):
    ...
    if t > 0:
        s2 = np.sqrt(t + 1.0) * 2
        return np.array([(R[2, 1] - R[1, 2]) / s2, (R[0, 2] - R[2, 0]) / s2,
                         (R[1, 0] - R[0, 1]) / s2, 0.25 * s2])
    i = int(np.argmax([R[0, 0], R[1, 1], R[2, 2]]))
    if i == 0:
        s2 = np.sqrt(1 + R[0, 0] - R[1, 1] - R[2, 2]) * 2
        return np.array([0.25 * s2, (R[0, 1] + R[1, 0]) / s2,
                         (R[0, 2] + R[2, 0]) / s2, (R[2, 1] - R[1, 2]) / s2])
    if i == 1:
        s2 = np.sqrt(1 + R[1, 1] - R[0, 0] - R[2, 2]) * 2
        return np.array([(R[0, 1] + R[1, 0]) / s2, 0.25 * s2,
                         (R[1, 2] + R[2, 1]) / s2, (R[0, 2] - R[2, 0]) / s2])
    s2 = np.sqrt(1 + R[2, 2] - R[0, 0] - R[1, 1]) * 2
    return np.array([(R[0, 2] + R[2, 0]) / s2, (R[1, 2] + R[2, 1]) / s2,
                     0.25 * s2, (R[1, 0] - R[0, 1]) / s2])


def _decode_packed_rotation(words):
    """Axis-angle rotation packed into 8 bytes per key.
        ...
        (bits 8..15, 28..31, 44..47, 60..63 are constant tags)

        theta = A * pi / 360            (half angle: A/2 degrees)
        axis  = normalise(-int12(B), -int12(C), -int12(D))
        q     = (axis * sin(theta), cos(theta))

    Verified against the inverse-bind matrices of 569 bones from 13 files:
    median angular error 0.467 deg, 99.1% below 1 deg. The residual is exactly
    the 1-degree quantisation of the 8-bit angle.
    """
    def s12(v):
        return np.where(v >= 2048, v.astype(np.int64) - 4096, v.astype(np.int64))
    a = (words & np.uint64(0xFF)).astype(np.float64)
    ax = -s12((words >> np.uint64(16)) & np.uint64(0xFFF)).astype(np.float64)
    ay = -s12((words >> np.uint64(32)) & np.uint64(0xFFF)).astype(np.float64)
    az = -s12((words >> np.uint64(48)) & np.uint64(0xFFF)).astype(np.float64)
    v = np.stack([ax, ay, az], axis=-1)
    ln = np.linalg.norm(v, axis=-1, keepdims=True)
    v = v / np.where(ln < 1e-9, 1.0, ln)
    th = a * (np.pi / 360.0)
    # stored CONJUGATED: the rest of the importer conjugates again, matching
    # the row-vector convention the format uses everywhere else
    return np.concatenate([-v * np.sin(th)[..., None],
                           np.cos(th)[..., None]], axis=-1).astype(np.float32)
...
def _unit_quaternion_score(addrs, B2, frames, sample=24, rot_only=False):
    """(share of exactly-unit keys, median deviation from unit length).

    rot_only: the addresses already point at the rotation track.
    """
    devs = []
    ...
    for k in range(0, len(addrs), step):
        if addrs[k] is None:
            continue
        o = addrs[k] + (0 if rot_only else frames * 12)

...
class EG3D_OT_import(Operator, ImportHelper):
    ...
    flip_uv_v: BoolProperty(
        name="Flip UV V", default=True,
        description="The files store V pointing down (DirectX convention) "
                    "while Blender's V points up, so the coordinate has to be "
                    "flipped. Verified against the real textures of "
                    "336_skin1.model. Only turn this off if a texture comes "
                    "out upside down")
    ...
    import_animations: BoolProperty(
        ...
        description="Import all clips as actions. Also matters for placement: "
                    "props such as weapons sit at the origin in the bind pose "
                    "and are only moved into the character's hands by the "
                    "animation, so without a clip they look missing")
    ...
    assign_action: BoolProperty(
        name="Assign first action", default=True,
        description="Apply the first clip right after import. Turn this off to "
                    "keep the bind pose -- but note that weapons and other "
                    "props then lie at the origin, because only the animation "
                    "puts them in the character's hands")
    import_rigid_props: BoolProperty(
        name="Rigid props", default=False,
        description="Import geometry that hangs off a node without any bone "
                    "weights (weapons, effect cards). Their coordinate space "
                    "is NOT understood yet -- on 039_skin8.model they land at "
                    "y 6.7..8.4 while the body ends at y 1.0, so they fly "
                    "around during animation. Off by default until that is "
                    "solved")
    ignore_zero_scale: BoolProperty(
        name="Ignore zero-scale bones", default=False,
        description="Some props are hidden by scaling their bone to almost "
                    "zero instead of by a visibility flag. On 336_skin1.model "
                    "the two sword bones sit at scale 0.001 in all 31 clips, "
                    "which collapses the weapon and everything parented under "
                    "it. Turn this on to force such scale keys back to 1 and "
                    "see the hidden geometry. This deliberately deviates from "
                    "the file")
    zero_scale_threshold: FloatProperty(
        name="Zero-scale threshold", default=0.01, min=0.0, max=1.0,
        description="Scale keys at or below this value count as hidden")
    fps: FloatProperty(
        name="FPS", default=30.0, min=1.0, max=240.0,
        description="Key times in the file are rounded milliseconds (33, 66, "
                    "100) that are meant as frames: 33 ms * 30 / 1000 = 0.99, "
                    "i.e. frame 1. Keys are placed at those snapped frames, "
                    "so no resampling happens and the drift is gone")
    ...
    def _handedness(self, model, is_x, log):
        ...
        try:
            ...
            # Falling back to Mirror X used to mirror rigs that simply have no
            # L/R bone names -- 083_b.x is a turtle with bones called Bone01.
            # Flipping the winding is the safe default: it fixes the facing
            # without changing left and right.
            log.warn("handedness detection failed (%s), falling back to a "
                     "winding flip", exc)
            return False, True
    ...
    def _import_one(self, context, path):
        ...
        opts = {
            ...
            "import_rigid_props": self.import_rigid_props,
            "ignore_zero_scale": self.ignore_zero_scale,
            "zero_scale_threshold": self.zero_scale_threshold,
            "fps": self.fps,
...
class EG3D_PT_include(_EG3DPanel):
    ...
    def draw(self, context):
        ...
        col.prop(op, "import_rigid_props")
        col.prop(op, "anim_align")
        col.prop(op, "ignore_zero_scale")
        sub = col.row()
        sub.enabled = op.ignore_zero_scale
        sub.prop(op, "zero_scale_threshold")
        col.prop(op, "fps")

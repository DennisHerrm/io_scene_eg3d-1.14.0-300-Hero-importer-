...
try:
    from .eg3d_parse import (BOOLEAN_CHANNELS, ATTR_POSITION, ATTR_NORMAL, ATTR_TANGENT,
...
def set_linear_interpolation(action):
    """Force every key of an action to LINEAR.

    Blender inserts keyframes as BEZIER with auto-clamped handles. Those
    handles are derived from the neighbouring keys, so wherever the key
    spacing changes the curve overshoots between them -- the value leaves the
    range its own keys define. The source data has no such easing: it is a
    plain sequence of sampled poses.

    This shows up worst where one bone in a chain is keyed densely and another
    sparsely. In 336_skin1.model the leg bones carry 46 rotation keys at 33 ms
    while 'Bip001 Spine' has only 6, at 0, 266, 732, 831 and 1331 ms. Every
    Spine key made the foot twitch, because Bezier handles pulled the curve
    past the key values on both sides.
    """
    n = 0
...
def _sample_at_frame(frames_of_key, values, f):
    """Channel value at frame f, interpolated ON THE FRAME GRID.

    Two things have to be true at once, and getting only one of them wrong is
    visible:

    * At a frame where this channel HAS a key, the value must be that key
      exactly. Sampling at f * 1000/fps instead drifts against the file's 33 ms
      grid and lands between keys -- that made walk cycles sway.
    * Between its own keys the value must be INTERPOLATED. Holding the last key
      turns sparse channels into a staircase: in bat_idle one channel has a key
      on only 9% of the frames, in dance one has 2%. That made every clip
      except the dense run cycles jitter.

    Interpolating between the snapped key frames satisfies both: exact on the
    keys, smooth in between, and no millisecond drift.
    """
    n = len(frames_of_key)
    if n == 0:
        return values[0]
    if f <= frames_of_key[0]:
        return values[0]
    if f >= frames_of_key[-1]:
        return values[-1]
    i = int(np.searchsorted(frames_of_key, f, side="right"))
    i = max(1, min(i, n - 1))
    f0, f1 = frames_of_key[i - 1], frames_of_key[i]
    a, b = values[i - 1], values[i]
    if f1 <= f0:
        return a
    u = float(f - f0) / float(f1 - f0)
    if len(a) == 4:
...
def _node_subtree(model, root):
    out = {root}
    stack = [root]
    while stack:
        j = stack.pop()
        for c in model.nodes[j].children:
            if 0 <= c < len(model.nodes) and c not in out:
                out.add(c)
                stack.append(c)
    return out


def keyframe_visibility(model, clip, objects_by_node, log):
    """Key object visibility from the boolean channels.

    A clip can switch whole node subtrees on and off. In 039_skin8.model the
    node '144_skin10_dance' carries the entire second character -- 16636
    vertices across groups 2 to 8 -- and is ON only during the 'dance' clip.
    Ignoring these channels leaves that character visible in all 17 clips,
    overlapping the first one.
    """
    n = 0
    for ch in clip.channels:
        if str(ch.kind) not in BOOLEAN_CHANNELS:
            continue
        if not (0 <= ch.node < len(model.nodes)):
            continue
        targets = _node_subtree(model, ch.node)
        objs = [o for nd, lst in objects_by_node.items() if nd in targets
                for o in lst]
        if not objs:
            continue
        times = np.asarray(ch.times_ms, dtype=np.float64)
        vals = np.asarray(ch.values, dtype=np.float64).ravel()
        for obj in objs:
            for t, v in zip(times, vals):
                hidden = v < 0.5
                obj.hide_viewport = hidden
                obj.hide_render = hidden
                f = t / 1000.0 * FPS_HINT[0] + 1.0
                obj.keyframe_insert("hide_viewport", frame=f)
                obj.keyframe_insert("hide_render", frame=f)
            n += 1
    if n and log:
        log.info("clip %-22s %d object(s) get keyed visibility", clip.name, n)
    return n
...
def import_animations(context, model, arm_obj, bone_names, rest, opts, log,
    ...
    xform = opts["xform"]
    mirror = opts["mirror"]
    # The bind matrices carry a uniform scale (1.688 on 039_skin8.model), but
    # a Blender bone CANNOT: edit_bones are built from head, tail and roll, so
    # bone.matrix_local is always orthonormal -- build_armature drops the scale
    # with to_quaternion(). The animation has to be computed against the SAME
    # rest, otherwise every local translation is off by the parent's scale
    # factor. Measured on 039_skin8.model: rotations matched exactly but rest
    # positions differed by up to 0.573 units on a 4.28 unit model, which is
    # what made walk cycles sway.
    # Blender normalises rest matrices internally (normalize_m4 in
    # armature.c), so bone.matrix_local can never hold the scale the bind
    # matrices carry -- 1.6881 on 039_skin8.model. Two things follow:
    #
    #   rest_orth  is what Blender actually uses for the deform,
    #   rest_raw   is the true bind including the scale.
    #
    # The deform Blender computes is  pose_world @ rest_orth^-1, but the
    # correct one is  anim_world @ rest_raw^-1. Equating the two gives a
    # per-bone correction applied FROM THE RIGHT:
    #
    #   pose_world = anim_world @ rest_raw^-1 @ rest_orth
    #
    # The 3ds Max plugin of this project does the same and calls it
    # "Korrektur von RECHTS". Without it the animation is clean but the mesh
    # deforms at the wrong scale.
    rest_raw = {i: xform @ rest[i] @ mirror for i in rest}
    rest_world = {}
    correction = {}
    singular_bind = []
    for i in rest:
        w = rest_raw[i]
        o = w.to_quaternion().to_matrix().to_4x4()
        o.translation = w.translation
        rest_world[i] = o
        # inverted_safe() returns identity for a singular matrix, which would
        # silently produce a wrong correction. 263_skin5.model has one.
        # 263_skin5.model contains two all-zero bind matrices (Bone043 among
        # them) -- data damage in the file, not a decoding problem.
        if abs(w.determinant()) < 1e-9:
            correction[i] = Matrix.Identity(4)
            rest_world[i] = Matrix.Identity(4)
            singular_bind.append(i)
        else:
            correction[i] = w.inverted() @ o
    bone_parent = {}
    for i in bone_names:
        p = model.nodes[i].parent
    ...
    if singular_bind:
        log.warn("%d bone(s) have a singular bind matrix; their scale "
                 "correction is skipped (the file cannot express one)",
                 len(singular_bind))
    ...
    zero_thresh = (opts.get("zero_scale_threshold", 0.01)
                   if opts.get("ignore_zero_scale") else None)
    zeroed = set()
    ...
    for clip in model.clips:
        ...
        # Keys sit at the SOURCE key times, snapped to whole frames -- not on
        # a resampled grid. The files store key times as rounded milliseconds
        # (33, 66, 100), which are meant as frames: 33 ms * 30 fps / 1000 =
        # 0.99, i.e. frame 1. Sampling at f/fps instead lands between the
        # source keys and adds a second interpolation on top of Blender's own,
        # which is what made walk cycles sway. The 3ds Max importer does the
        # same thing and comes out clean.
        def snap(ms):
            return int(round(ms * fps / 1000.0))

        # Two source keys can round to the SAME frame, and then one is lost.
        # Measured over 50 files: at 30 fps a median of 4.45% of keys collide
        # and up to 14.7% in the worst file, at 60 fps 0.28%, at 120 fps none.
        # Report it so the loss is visible instead of silent.
        n_keys = n_lost = 0
        for chans in by_node.values():
            for ch in chans.values():
                if ch.times_ms is None or len(ch.times_ms) < 2:
                    continue
                kf = [snap(float(x)) for x in ch.times_ms]
                n_keys += len(kf)
                n_lost += len(kf) - len(set(kf))
        if n_lost and log:
            share = 100.0 * n_lost / max(n_keys, 1)
            fn = log.warn if share > 2.0 else log.info
            fn("clip %-22s %d of %d source keys (%.1f%%) fall on a frame that "
               "already has one and are dropped -- raise FPS to keep them",
               clip.name, n_lost, n_keys, share)

        frames_of_node = {}
        key_frames = {}          # channel -> snapped frame per key
        for i, chans in by_node.items():
            fs = set()
            for ch in chans.values():
                if ch.times_ms is None:
                    continue
                kf = np.array([snap(float(ms)) for ms in ch.times_ms],
                              dtype=np.int64)
                key_frames[id(ch)] = kf
                fs.update(int(x) for x in kf)
            frames_of_node[i] = fs

        # A bone needs a key wherever any node between it and its bone parent
        # has one, because its local matrix depends on that whole chain.
        frames_of_bone = {}
        for i in bone_names:
            p = bone_parent[i]
            fs = set()
            ...
            guard = 0
            while j >= 0 and j != p and guard < 512:
                fs |= frames_of_node.get(j, set())
                j = model.nodes[j].parent
                guard += 1
            frames_of_bone[i] = fs

        all_frames = sorted({f for fs in frames_of_bone.values() for f in fs}
                            | {0})
        all_frames = [f for f in all_frames if f <= opts["max_frames"]]
        nframes = (max(all_frames) + 1) if all_frames else 2
        ...
        for f in all_frames:
            ...
            for i, n in enumerate(model.nodes):
                ...
                if c is not None and id(c) in key_frames:
                    loc = tuple(_sample_at_frame(key_frames[id(c)], c.values, f)[:3])
                c = chans.get(CH_ROTATION)
                if c is not None and id(c) in key_frames:
                    v = _sample_at_frame(key_frames[id(c)], c.values, f)
                    if len(v) >= 4:
                        rot = tuple(v[:4])
                c = chans.get(CH_SCALE)
                if c is not None and id(c) in key_frames:
                    v = _sample_at_frame(key_frames[id(c)], c.values, f)[:3]
                    if zero_thresh is not None and min(v) <= zero_thresh:
                        zeroed.add(i)
                        v = (1.0, 1.0, 1.0)
                    scl = tuple(v)
            ...
            for i, bname in bone_names.items():
                if f not in frames_of_bone.get(i, ()) and f != 0:
                    continue
                pb = arm_obj.pose.bones.get(bname)
                if pb is None:
                    continue
                p = bone_parent[i]
                # pose world with the right-hand correction, then the local
                # matrix against the parent's corrected pose world
                pw_i = (root_space @ world[i] @ mirror) @ correction[i]
                if p >= 0:
                    pw_p = (root_space @ world[p] @ mirror) @ correction[p]
                    anim_local = pw_p.inverted_safe() @ pw_i
                else:
                    anim_local = pw_i
                pb.matrix_basis = rest_local[i].inverted_safe() @ anim_local
                if frames_of_bone.get(i):
        ...
        if objects_by_node:
            FPS_HINT[0] = fps
            keyframe_visibility(model, clip, objects_by_node, log)
    ...
    if zeroed and log:
        log.warn("%d bone(s) had scale keys at or below %.4f and were forced "
                 "back to 1: %s. Geometry hidden by the engine this way is now "
                 "visible, which deliberately deviates from the file",
                 len(zeroed), zero_thresh,
                 ", ".join(sorted(model.nodes[i].name for i in zeroed)[:6]))
    ...
    for inst in model.instances:
        for sm in model.groups[inst["group"]]:
            ...
            if opts["skip_billboards"] and mat_name.endswith("_c"):
                ...
                continue
            if inst["palette"] is None and not opts.get("import_rigid_props"):
                log.info("submesh %s skipped: rigid prop, its coordinate space "
                         "is not understood yet (enable 'Rigid props' to see "
                         "it anyway)", sm.name)
                continue
            ...
            objects.append(obj)
            objects_by_node.setdefault(inst["node"], []).append(obj)
...
def _x_rest_matrix(bone, frame, zero_thresh=None):
    ...
    if bone.scale is not None:
        sc = bone.scale[frame]
        if zero_thresh is not None and min(float(x) for x in sc) <= zero_thresh:
            sc = (1.0, 1.0, 1.0)
...
def _x_animations(context, model, arm_obj, bone_names, rest_raw, opts, log):
    ...
    if zero_thresh is not None:
        hit = [b.name for b in model.bones
               if b.scale is not None
               and float(np.asarray(b.scale).min()) <= zero_thresh]
        if hit:
            log.warn("%d bone(s) have scale keys at or below %.4f and are "
                     "forced back to 1: %s", len(hit), zero_thresh,
                     ", ".join(hit[:6]))
    mirror = opts["mirror"]
    arm_obj.animation_data_create()
    ...
    # Same rule as the .model path: Blender bones are orthonormal, so the rest
    # used for the animation must be too. .x rest matrices carry scale when the
    # file has per-frame scale tracks (045.x, 063_skin7.x).
    # NOTE: in this path rest_raw is a LIST indexed by bone number, not a dict
    # like in the .model path. Calling .items() on it raised AttributeError and
    # killed the whole import before a single action was built (013.x).
    rest_orth = {}
    correction = {}
    for i in range(len(rest_raw)):
        M = rest_raw[i]
        o = M.to_quaternion().to_matrix().to_4x4()
        o.translation = M.translation
        rest_orth[i] = o
        correction[i] = (Matrix.Identity(4) if abs(M.determinant()) < 1e-9
                         else M.inverted() @ o)
    rest_local = {}
    for i in bone_names:
        p = parent[i]
        rest_local[i] = (rest_orth[p].inverted_safe() @ rest_orth[i]
                         if p in bone_names else rest_orth[i].copy())
    ...
    for clip in model.clips:
        ...
        for f in range(n):
            src = first + f
            world = {i: _x_rest_matrix(model.bones[i], src, zero_thresh)
                     for i in bone_names}
            for i in vis_bones:
                pb = arm_obj.pose.bones.get(bone_names[i])
                if pb is None:
            ...
            for i, bn in bone_names.items():
                ...
                if pb is None:
                    continue
                p = parent[i]
                pw_i = world[i] @ correction[i]
                if p in bone_names:
                    anim_local = (world[p] @ correction[p]).inverted_safe() @ pw_i
                else:
                    anim_local = pw_i
